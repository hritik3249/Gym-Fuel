  ...
  function handleAddLog() {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        setWeightLogs((c) => c.filter((l) => l.id !== newLog.id));
      }
    });
  }

  const weeklyChange = weekAgo && latest
    ? (latest.weightKg - weekAgo.weightKg).toFixed(1)
    : null;
  const monthlyChange = monthStart && latest && monthStart.id !== latest.id
    ? (latest.weightKg - monthStart.weightKg).toFixed(1)
    : null;
    ...
    <div className="container grid gap-4 py-4 sm:py-6">
      <section className="surface-glass rounded-lg p-4 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {([
            ["Current weight", latest ? `${latest.weightKg}kg` : "—", Scale],
            ["Weekly change", weeklyChange ? `${weeklyChange}kg` : "—", TrendingDown],
            ["Monthly change", monthlyChange ? `${monthlyChange}kg` : "—", Ruler]
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
        ...
        <Card>
          ...
          <CardContent className="grid gap-3">
            ...
            <div className="grid gap-2">
              <Label htmlFor="body-fat">Body fat % <span className="text-muted-foreground">(optional)</span></Label>
              <Input
                id="body-fat"
                value={bodyFat}
            ...
            <div className="grid gap-2">
              <Label htmlFor="waist">Waist cm <span className="text-muted-foreground">(optional)</span></Label>
              <Input
                id="waist"
                value={waist}
