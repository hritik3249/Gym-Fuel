"use server";

import { createClient } from "@/lib/supabase/server";
import { getFoodsPageData, getWeightPageData, getAnalyticsPageData, getDashboardSnapshot } from "@/lib/queries/dashboard";
import { defaultGoals } from "@/lib/nutrition";
import type { SettingsProfile } from "@/components/settings-view";
import type { Goal } from "@/lib/types";
import type { ActivityLevel, FitnessGoal, Gender } from "@/lib/calculator";

export { getFoodsPageData, getWeightPageData, getAnalyticsPageData, getDashboardSnapshot };

function safeNum(v: unknown, fallback: number): number {
  const n = Number(v);
  return Number.isFinite(n) && n > 0 ? n : fallback;
}

function mapGoals(row: Record<string, unknown> | null): Goal {
  if (!row) return defaultGoals;
  return {
    calories:       safeNum(row.calories,         defaultGoals.calories),
    protein:        safeNum(row.protein,           defaultGoals.protein),
    carbs:          safeNum(row.carbs,             defaultGoals.carbs),
    fat:            safeNum(row.fat,               defaultGoals.fat),
    waterMl:        safeNum(row.water_ml,          defaultGoals.waterMl),
    targetWeightKg: safeNum(row.target_weight_kg,  defaultGoals.targetWeightKg),
  };
}

export type SettingsPageData = {
  profile: SettingsProfile;
  goals: Goal;
  reminders: { breakfast: boolean; lunch: boolean; dinner: boolean; hydration: boolean; weekly: boolean };
  isNewUser: boolean;
};

export async function getSettingsPageData(): Promise<SettingsPageData> {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { profile: emptyProfile(), goals: defaultGoals, reminders: emptyReminders(), isNewUser: true };

  const [{ data: goalsData }, { data: profileData }] = await Promise.all([
    supabase.from("goals").select("*").eq("user_id", user.id).single(),
    supabase.from("profiles").select("*").eq("id", user.id).single()
  ]);

  const isNewUser = !profileData && !goalsData;

  const profile: SettingsProfile = {
    displayName:   profileData?.display_name    ?? user.user_metadata?.full_name ?? user.email?.split("@")[0] ?? "",
    age:           profileData?.age             ?? "",
    gender:        (profileData?.gender         ?? "male") as Gender,
    heightCm:      profileData?.height_cm       ?? "",
    weightKg:      profileData?.current_weight_kg ?? "",
    activityLevel: (profileData?.activity_level ?? "moderate") as ActivityLevel,
    fitnessGoal:   (profileData?.fitness_goal   ?? "maintain") as FitnessGoal,
  };

  const reminders = {
    breakfast: profileData?.reminder_breakfast ?? false,
    lunch:     profileData?.reminder_lunch     ?? false,
    dinner:    profileData?.reminder_dinner    ?? false,
    hydration: profileData?.reminder_hydration ?? false,
    weekly:    profileData?.reminder_weekly    ?? false,
  };

  return { profile, goals: mapGoals(goalsData), reminders, isNewUser };
}

function emptyProfile(): SettingsProfile {
  return { displayName: "", age: "", gender: "male", heightCm: "", weightKg: "", activityLevel: "moderate", fitnessGoal: "maintain" };
}

function emptyReminders() {
  return { breakfast: false, lunch: false, dinner: false, hydration: false, weekly: false };
}
