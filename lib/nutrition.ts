...
export const nutrientTargets: Omit<Nutrients, "calories" | "protein" | "carbs" | "fat" | "fiber"> & {
  fiber: number;
} = {
  fiber: 30,
  iron: 18,
...
export function sumEntries(entries: FoodEntry[]): Nutrients {
  return entries.reduce(
    (total, entry) => ({
      ...
      iron: total.iron + entry.iron,
      calcium: total.calcium + entry.calcium,
      magnesium: total.magnesium + entry.magnesium,
      zinc: total.zinc + entry.zinc,
      potassium: total.potassium + entry.potassium,
      sodium: total.sodium + entry.sodium,
      vitaminD: total.vitaminD + entry.vitaminD,
